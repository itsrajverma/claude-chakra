...
class TestSettings:
    ...
    def test_default_claude_workspace_uses_chakra_home(self, monkeypatch, tmp_path):
        """Unset CLAUDE_WORKSPACE stores agent data under ~/.chakra."""
        from config.settings import Settings
    ...
    def test_server_log_path_uses_chakra_home(self, monkeypatch, tmp_path):
        ...
        assert server_log_path() == tmp_path / ".chakra" / "logs" / "server.log"
    ...
    def test_blank_claude_workspace_uses_chakra_home(self, monkeypatch, tmp_path):
    ...
    def test_api_keys_for_prefers_json_list(self, monkeypatch):
        """``*_API_KEYS`` JSON list overrides the legacy single key."""
        from config.settings import Settings

        monkeypatch.setenv("NVIDIA_NIM_API_KEY", "legacy-single")
        monkeypatch.setenv(
            "NVIDIA_NIM_API_KEYS", '["nvapi-aaa","nvapi-bbb","nvapi-ccc"]'
        )
        monkeypatch.setitem(Settings.model_config, "env_file", ())

        settings = Settings()
        assert settings.api_keys_for("nvidia_nim") == (
            "nvapi-aaa",
            "nvapi-bbb",
            "nvapi-ccc",
        )

    def test_api_keys_for_falls_back_to_single_key(self, monkeypatch):
        """Legacy ``*_API_KEY`` is exposed as a single-element pool."""
        from config.settings import Settings
        ...
        settings = Settings()
        assert settings.api_keys_for("open_router") == ("sk-or-legacy",)

    def test_api_keys_for_accepts_comma_separated_list(self, monkeypatch):
        """Comma-separated values are also accepted for ergonomic shell entry."""
        from config.settings import Settings

        monkeypatch.setenv("DEEPSEEK_API_KEYS", "ds-1, ds-2 , ,ds-3")
        monkeypatch.setitem(Settings.model_config, "env_file", ())

        settings = Settings()
        assert settings.api_keys_for("deepseek") == ("ds-1", "ds-2", "ds-3")

    def test_api_keys_for_returns_empty_when_unconfigured(self, monkeypatch):
        from config.settings import Settings
        ...
        settings = Settings()
        assert settings.api_keys_for("kimi") == ()
