...
def serve() -> None:
    """Start the FastAPI server (registered as `chakra-server` script)."""
...
def _admin_browser_open_enabled() -> bool:
    ...
    Reads ``CHAKRA_OPEN_BROWSER`` (preferred) with ``FCC_OPEN_BROWSER`` as a
    legacy fallback. Set to ``0``/``false``/``no`` to suppress the popup.
    """

    raw = os.environ.get("CHAKRA_OPEN_BROWSER") or os.environ.get(
...
def _schedule_open_admin_browser(settings: Settings) -> None:
    ...
    threading.Thread(
        target=open_when_ready, name="chakra-open-admin-browser", daemon=True
    ).start()
...
def init() -> None:
    """Scaffold config at ~/.chakra/.env (registered as `chakra-init`)."""
    ...
    if migrated_from is not None:
        ...
        print(
            "Edit it to set your API keys and model preferences, then run: chakra-server"
        )
        return
...
def _claude_child_env(
    ...
    for key, value in settings.claude_code_launch_env().items():
...
def launch_claude(argv: Sequence[str] | None = None) -> None:
    """Launch Claude Code with Claude Chakra proxy environment variables."""

    settings = get_settings()
    proxy_root_url = local_proxy_root_url(settings)
    if error := _preflight_proxy(proxy_root_url):
        print(
            f"Claude Chakra proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
        print("Start it in another terminal with: chakra-server", file=sys.stderr)
        raise SystemExit(1)
