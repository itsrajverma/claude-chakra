...
class OpenAIChatTransport(BaseProvider):
    ...
    def __init__(
        ...
        *,
        ...
        pool_keys = config.api_keys or (api_key,)
        ...
        self._client = self._client_for_key(api_key)

    def _client_for_key(self, api_key: str) -> AsyncOpenAI:
        """Return a cached :class:`AsyncOpenAI` for ``api_key``, creating it lazily."""
    ...
    async def cleanup(self) -> None:
        """Release HTTP client resources for every per-key client."""
    ...
    async def _create_with_key_rotation(self, create_body: dict) -> Any:
        """Open a stream, rotating to the next live key on 429 before backing off.

        Each pool key is tried at most once per request. When 429 is raised we
        mark the current key as cooling and pick the next live key; this keeps
        an in-flight request unblocked while a key is over quota. If every key
        ...
        fallback_client = self._client_for_key(fallback_key)
        try:
    ...
    async def _create_stream(self, body: dict) -> tuple[Any, dict]:
        ...
        try:
            ...
            stream = await self._create_with_key_rotation(create_body)
            return stream, body
        except Exception as error:
            ...
            stream = await self._create_with_key_rotation(create_retry_body)
