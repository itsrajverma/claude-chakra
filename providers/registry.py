...
def _credential_for(descriptor: ProviderDescriptor, settings: Settings) -> str:
    ...
    if descriptor.credential_attr:
        # Prefer the multi-key pool's first key when configured.
        keys = _credentials_for(descriptor, settings)
        if keys:
            return keys[0]
...
def _credentials_for(
    ...
    """Return all configured credentials for ``descriptor``.

    Static-credential providers (Ollama, LM Studio, llama.cpp) return a
    single-element tuple. Authenticated providers consult ``Settings.api_keys_for``
    which prefers the JSON-list ``*_API_KEYS`` value, falling back to the legacy
    ``*_API_KEY``.
    """
    if descriptor.static_credential is not None:
        return (descriptor.static_credential,)
    return settings.api_keys_for(descriptor.provider_id)
...
def build_provider_config(
    ...
    credentials = _credentials_for(descriptor, settings)
    primary = credentials[0] if credentials else ""
    _require_credential(descriptor, primary)
