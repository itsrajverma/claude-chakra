...
from providers.rate_limit import GlobalRateLimiter, retryable_upstream_status
...
class AnthropicMessagesTransport(BaseProvider):
    ...
    def __init__(
        ...
        *,
        ...
        self._key_pool = KeyPool(pool_keys, scope=provider_name.lower())
    ...
    def _request_headers(self, *, api_key: str | None = None) -> dict[str, str]:
        ...
        Subclasses that authenticate the request should read ``api_key``
        (falling back to ``self._api_key``) so key rotation in
        :meth:`stream_response` can supply a different pool key per attempt.
        """
        return {"Content-Type": "application/json"}

    def _resolve_request_api_key(self, api_key: str | None) -> str:
        return api_key if api_key is not None else self._api_key
    ...
    async def _send_stream_request(
        ...
        """Create a streaming messages response with the supplied pool key."""
    ...
    async def _open_stream_with_key_rotation(
        self, body: dict, *, req_tag: str
    ) -> httpx.Response:
        """Open the upstream stream, rotating the pool key on 429.

        Mirrors :meth:`providers.openai_compat.OpenAIChatTransport._create_with_key_rotation`:
        each key in the pool is tried at most once before falling back to the
        global rate limiter's reactive backoff. ``_raise_for_status`` is called
        on non-200 responses so the upstream body is closed and the underlying
        ``httpx.HTTPStatusError`` is observed by the rotation check.
        """

        async def _send_with_key(api_key: str) -> httpx.Response:
            send_response = await self._send_stream_request(body, api_key=api_key)
        ...
        while attempts_remaining > 0:
            ...
            api_key = self._key_pool.next_live_key()
            try:
                return await _send_with_key(api_key)
        ...
        fallback_key = self._key_pool.next_live_key()
        try:
            return await self._global_rate_limiter.execute_with_retry(
                _send_with_key, fallback_key
            )
        except Exception:
