...
class ProviderConfig(BaseModel):
    ...
    ``api_key`` remains the primary key used by single-key providers; when a
    pool is configured via ``api_keys`` it equals ``api_keys[0]`` so the
    existing single-key transport paths keep working unchanged.
    """
