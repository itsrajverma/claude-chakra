...
def _parse_api_keys_json(raw: Any) -> tuple[str, ...]:
    """Parse a `*_API_KEYS` env value into a tuple of keys.

    Accepts a JSON list (``["k1","k2"]``), a comma-separated string, or an empty
    value. Whitespace and empty entries are dropped. Used for multi-key
    providers so a single env var can hold several keys for round-robin
    rotation in :class:`providers.key_pool.KeyPool`.
    """
    if raw is None:
        return ()
    ...
    if text.startswith("["):
        try:
            ...
            raise ValueError(
                f"*_API_KEYS must be a JSON list of strings, got: {text!r}"
            ) from exc
        if not isinstance(data, list):
            raise ValueError("*_API_KEYS JSON value must be a list of strings")
        return tuple(
...
def _env_files() -> tuple[Path, ...]:
    ...
    explicit = os.environ.get("CHAKRA_ENV_FILE") or os.environ.get("FCC_ENV_FILE")
    if explicit:
...
class Settings(BaseSettings):
    ...
    deepseek_api_keys: str = Field(default="", validation_alias="DEEPSEEK_API_KEYS")

    # ==================== Kimi Config ====================
    kimi_api_key: str = Field(default="", validation_alias="KIMI_API_KEY")
    kimi_api_keys: str = Field(default="", validation_alias="KIMI_API_KEYS")

    # ==================== Wafer Config ====================
    wafer_api_key: str = Field(default="", validation_alias="WAFER_API_KEY")
    wafer_api_keys: str = Field(default="", validation_alias="WAFER_API_KEYS")

    # ==================== OpenCode Zen Config ====================
    opencode_api_key: str = Field(default="", validation_alias="OPENCODE_API_KEY")
    opencode_api_keys: str = Field(default="", validation_alias="OPENCODE_API_KEYS")

    # ==================== Z.ai Config ====================
    zai_api_key: str = Field(default="", validation_alias="ZAI_API_KEY")
    zai_api_keys: str = Field(default="", validation_alias="ZAI_API_KEYS")
    ...
    def api_keys_for(self, provider_id: str) -> tuple[str, ...]:
        """Return the configured API key list for ``provider_id``.

        Prefers ``*_API_KEYS`` (multi-key, JSON list or comma-separated).
        Falls back to the single ``*_API_KEY`` value for backwards
        compatibility. Returns an empty tuple when nothing is configured.
        """
        plural_attr = f"{provider_id}_api_keys"
        singular_attr = f"{provider_id}_api_key"
        plural_raw = getattr(self, plural_attr, "")
        plural = _parse_api_keys_json(plural_raw)
        if plural:
            return plural
        single = getattr(self, singular_attr, "")
