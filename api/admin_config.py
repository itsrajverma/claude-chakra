...
FIELDS: tuple[ConfigFieldSpec, ...] = (
    ConfigFieldSpec(
        ...
        secret=True,
        description="Single NVIDIA NIM key. For multi-key rotation, use NVIDIA NIM API Keys below.",
    ),
    ConfigFieldSpec(
        "NVIDIA_NIM_API_KEYS",
        "NVIDIA NIM API Keys (pool)",
        "providers",
        "textarea",
        settings_attr="nvidia_nim_api_keys",
        secret=True,
        description=(
            'Optional. JSON list like ["nvapi-aaa","nvapi-bbb","nvapi-ccc"] or '
            "comma-separated. When set, requests round-robin across keys and "
            "rotate to the next live key on 429."
        ),
    ),
    ConfigFieldSpec(
        "OPENROUTER_API_KEY",
    ...
    ConfigFieldSpec(
        "OPENROUTER_API_KEYS",
        "OpenRouter API Keys (pool)",
        "providers",
        "textarea",
        settings_attr="open_router_api_keys",
        secret=True,
        description='JSON list (e.g. ["sk-or-aaa","sk-or-bbb"]) for round-robin rotation on 429.',
    ),
    ConfigFieldSpec(
        "DEEPSEEK_API_KEY",
        "DeepSeek API Key",
    ...
    ConfigFieldSpec(
        "DEEPSEEK_API_KEYS",
        "DeepSeek API Keys (pool)",
        "providers",
        "textarea",
        settings_attr="deepseek_api_keys",
        secret=True,
        description="JSON list of DeepSeek keys for rotation on 429.",
    ),
    ConfigFieldSpec(
        "KIMI_API_KEY",
        "Kimi API Key",
    ...
    ConfigFieldSpec(
        ...
        "providers",
        "textarea",
        settings_attr="kimi_api_keys",
        secret=True,
        description="JSON list of Kimi keys for rotation on 429.",
    ),
    ConfigFieldSpec(
        "WAFER_API_KEY",
        "Wafer API Key",
    ...
    ConfigFieldSpec(
        ...
        "providers",
        "textarea",
        settings_attr="wafer_api_keys",
        secret=True,
        description="JSON list of Wafer keys for rotation on 429.",
    ),
    ConfigFieldSpec(
        "OPENCODE_API_KEY",
    ...
    ConfigFieldSpec(
        "OPENCODE_API_KEYS",
        "OpenCode Zen API Keys (pool)",
        "providers",
        "textarea",
        settings_attr="opencode_api_keys",
        secret=True,
        description="JSON list of OpenCode Zen keys for rotation on 429.",
    ),
    ConfigFieldSpec(
        "ZAI_API_KEY",
        "Z.ai API Key",
    ...
    ConfigFieldSpec(
        ...
        "providers",
        "textarea",
        settings_attr="zai_api_keys",
        secret=True,
        description="JSON list of Z.ai keys for rotation on 429.",
    ),
    ConfigFieldSpec(
        "LM_STUDIO_BASE_URL",
        "LM Studio Base URL",
...
def explicit_env_path() -> Path | None:
    """Return the explicit env-file override path, when configured.

    Reads ``CHAKRA_ENV_FILE`` first and falls back to the legacy
    ``FCC_ENV_FILE`` so prior shells and scripts keep working.
    """
...
def render_env_file(values: Mapping[str, str], *, mask_secrets: bool = False) -> str:
    ...
    lines: list[str] = [
        "# Managed by Claude Chakra /admin.",
